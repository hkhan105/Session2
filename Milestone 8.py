...
# ret, frame = cam.read()
frame = cv2.imread("C:/Users/tajam/Downloads/image-removebg-preview (5).png")
...
if frame is not None:
    ...
    histImage = cv2.imread("C:/Users/tajam/Downloads/image-removebg-preview (5).png")  # read the image from file
    maskedHistIm = cv2.inRange(frame, np.array((0., 60., 32.)), np.array((180., 255., 255.)))
    hist = cv2.calcHist([frame], [0], maskedHistIm, [16], [0, 180])
    ...
    while True:
        ...
        if showBackProj:
            vis[:] = prob[..., np.newaxis]
        try:
            cv2.ellipse(vis, track_box, (255, 255, 0), 2)
        except:
